...
<div class="container">
    <form id="form-store" action="{{route('save.tienda')}}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        @if (count($tiendas) !== 0)
            @foreach ($tiendas as $tienda)
                @if ($tienda->panel !== null || !empty($tienda->tienda->panel))
                    ...
                    @else
                        <div id="insert-panel" style="width: 100%; height: 250px;">
                            <img src="{{route('my.panel', ['panel' => $tienda->tienda->panel])}}" alt='file-selected' width="100%" height="100%"/>
                        </div>
                    @endif
                @else
                        <div id="insert-panel" style="width: 100%; height: 250px;">
        ...
        <i style="color: #2fcece; position: absolute; top: 60%; left: 15%;" onclick="handleUploadFileImagen()" id="hidePenImagen" class="fas fa-pen"></i>
            @if (count($tiendas) !== 0)
                @foreach ($tiendas as $tienda)
                    ...
                    @else
                    @if ($tienda->tienda->imagen === null)
                    <div id="insert-image" style="width: 9%; height: 93px;">
                        <img src="http://amsolidaritas.com.ar/wp-content/themes/wellington/assets/images/default-slider-image.png" alt='file-selected' width="100%" height="100%"/>
                    </div>
                    @else
                    <div id="insert-image" style="width: 9%; height: 93px;">
                        <img src="{{route('my.imagen.tienda', ['imagen' => $tienda->tienda->imagen])}}" alt='file-selected' width="100%" height="100%"/>
                    </div>
                    @endif
                    <div class="ml-2">
                        <i style="color: #2fcece; position: absolute; top: 70%; left: 41%;" id="penTienda" class="fas fa-pen"></i>
                        <input style="width: 22rem;" type="text" class="form-control" name="tienda" value="{{$tienda->tienda->tienda}}" placeholder="Nombre de la Marca:">
                        <i style="color: #2fcece; position: absolute; top: 62%; left: 41%;" id="penSector" class="fas fa-pen"></i>
                        <input style="width: 22rem;" type="text" class="form-control mt-2" name="sector" value="{{$tienda->tienda->sector}}" placeholder="Sector:">
                    </div>
                    @endif
                @endforeach
            @else
                ...
                <div class="ml-2">
                    <i style="color: #2fcece; position: absolute; top: 70%; left: 41%;" id="penTienda" class="fas fa-pen"></i>
                    <input style="width: 22rem;" type="text" class="form-control" name="tienda" value="" placeholder="Nombre de la Marca:">
                    <i style="color: #2fcece; position: absolute; top: 62%; left: 41%;" id="penSector" class="fas fa-pen"></i>
                    <input style="width: 22rem;" type="text" class="form-control mt-2" name="sector" placeholder="Sector:">
                </div>
            @endif
        </div>
                    ...
                    <div class="d-flex m-2 p-2" style="background-color: #2fcece; width: 150px; height: 50px; border-radius: 10px;">
                        ...
                        <div class="d-flex flex-column ml-2">
                            <div class="w-100 h-100 d-flex text-white">
                                <input type="number" name="tiempo" placeholder="00" class="border-0 bg-transparent h-100" style="outline: 0px ;font-size: 36px ;width: 53px;">
